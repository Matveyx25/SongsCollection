...
import { requestSongs, searchSong, toggleIsFetching } from '../../redux/songs-reducer'
import { getSongs, getFetching, getResult } from '../../redux/songs-selectors'
import { useForm } from "react-hook-form";
import s from './SearchPage.module.scss'


const SearchAPIComponent = (props) => {
        const { register, handleSubmit } = useForm();

        const onSubmit = data => {
            props.searchSong(data.search)
        }
    
        return <>
            <form onSubmit={handleSubmit(onSubmit)}>
                <div className="search-wrapper">
                    <input type="text" placeholder="поиск песен" {...register("search")} onBlur={onSubmit} />
            ...
            {props.isFetching ?
                <Preloader />
                : null}
            {props.songs.length > 0 ? <Songs songs={props.songs} /> : <span className={s.notResult}>Ничего не найдено</span>}
        </>
...
let mapStateToProps = state => {
    return {
        songs: getResult(state),
        isFetching: getFetching(state)
    }
}

const SearchPageContainer = connect(mapStateToProps, { requestSongs, searchSong, toggleIsFetching })(SearchAPIComponent)
